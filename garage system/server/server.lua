...
AddEventHandler('bb-garages:server:setFirstData', function()
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bbvehicles`", function(vehicles)
        if vehicles[1] ~= nil then
            ...
            for _, vehicle in pairs(vehicles) do
                ...
                elseif vehicle.state == 'unknown' then
                    QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `citizenid` = '" .. vehicle.citizenid .. "' LIMIT 1")
                end

                if vehicle.fakeplate ~= nil and vehicle.fakeplate ~= '' then
                    QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `citizenid` = '" .. vehicle.citizenid .. "' LIMIT 1")
                end
            end
...
AddEventHandler('bb-garages:server:impoundVehicle', function(plate)
    ...
    QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. plate .. "' AND `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' LIMIT 1")
end)
...
AddEventHandler('bb-garages:server:parkVehicle', function(garage, typ, slots, plate, stats)
    ...
    if typ == 'garages' then
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT `props` FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(props)
            if props[1] ~= nil then
                ...
                QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'garage', `parking` = '" .. json.encode(jsonz) .. "' WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "'")
    ...
    elseif typ == 'houses' then
        ...
        QBCore.Functions.ExecuteSql(false, "SELECT `props` FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(props)
            if props[1] ~= nil then
                ...
                QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'house', `parking` = '" .. json.encode(jsonz) .. "' WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "'")
...
AddEventHandler('bb-garages:server:setVehicleOwned', function(props, stats, model)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    QBCore.Functions.ExecuteSql(false, "INSERT INTO `bbvehicles` (`citizenid`, `plate`, `model`, `props`, `stats`, `state`) VALUES ('" .. xPlayer.PlayerData.citizenid .. "', '" .. props.plate .. "', '" .. model .. "', '" .. json.encode(props) .. "', '" .. json.encode(stats) .. "', 'unknown')")
end)
...
AddEventHandler('bb-garages:server:updateProps', function(props)
    ...
    local xPlayer = QBCore.Functions.GetPlayer(src)
    QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'garage', `parking` = '" .. json.encode(jsonz) .. "' WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "'")
    QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` (`props`) VALUES ('" .. xPlayer.PlayerData.citizenid .. "', '" .. props.plate .. "', '" .. model .. "', '" .. json.encode(props) .. "', '" .. json.encode(stats) .. "', 'unknown')")
end)
...
AddEventHandler('bb-garages:server:vehiclePayout', function(garage, plate, price, typ)
    ...
    if typ ~= 'houses' then
        if xPlayer.PlayerData.money['cash'] >= price or (GaragesConfig['settings']['bank-payments'] == true and xPlayer.PlayerData.money['bank'] >= price) then
            QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(vehicle)
                if vehicle[1] ~= nil then
                    ...
                    if modelExists == true then
                        if xPlayer.Functions.RemoveMoney('cash', tonumber(price)) or (GaragesConfig['settings']['bank-payments'] == true and xPlayer.Functions.RemoveMoney('bank', tonumber(price))) then
                            QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `state` = 'unknown', `parking` = '' WHERE `id` = '" .. veh.id .. "'")
                            if typ == 'garages' then
...
QBCore.Functions.CreateCallback('bb-garages:server:getConfig', function(source, cb)
    if isAuthorized == true then
        cb(serverConfig, isAuthorized, 'http://quackcity.nl/garages/')
    else
        cb(serverConfig, isAuthorized)
    end
end)

QBCore.Functions.CreateCallback('bb-garages:server:getOwnedVehicles', function(source, cb, nearbyVehicles, freeSlots, name, keys)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)

    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "'", function(vehicles)
...
QBCore.Functions.CreateCallback('bb-garages:server:getImpoundedVehicles', function(source, cb, name)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)

    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `state` = 'impound'", function(vehicles)
        function payButton(plate)
...
QBCore.Functions.CreateCallback('bb-garages:server:isVehicleOwned', function(source, cb, plate)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `bbvehicles` WHERE `citizenid` = '" .. xPlayer.PlayerData.citizenid .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(result)
...
QBCore.Functions.CreateCallback('bb-garages:server:hasFines', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("addhousegarage", "BBGarages: Add garage to nearest house", {{name = 'slots', help = 'Slots'}}, true, function(source, args)
...
QBCore.Commands.Add('setslot', 'BB-Garages: Set House Garage Slot', {{name = 'index', help = 'Index'}}, true, function(source, args)
    local xPlayer = QBCore.Functions.GetPlayer(source)
	if xPlayer.PlayerData.job.name == GaragesConfig['settings']['housing']['realestate-job'] then
...
AddEventHandler('bb-garages:server:isPlayerVehicle', function(typ, plate, vehicle)
    if typ == 'STEAL' then
        QBCore.Functions.ExecuteSql(false, "SELECT `model` FROM `bbvehicles` WHERE `plate` = '" .. plate .. "' LIMIT 1", function(result)
            if result[1] ~= nil then
                QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `fakeplate` = '%' WHERE `plate` = '" .. plate .. "' AND `model` = '" .. result[1].model .. "' LIMIT 1")
    ...
    elseif typ == 'SET' then
        if networkVehicles[vehicle] ~= nil then
            if networkVehicles[vehicle][1] == plate then
                QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. plate .. "' LIMIT 1")
                networkVehicles[vehicle] = nil
            else
                QBCore.Functions.ExecuteSql(false, "UPDATE `bbvehicles` SET `fakeplate` = '" .. plate .. "' WHERE `plate` = '" .. networkVehicles[vehicle][1] .. "' LIMIT 1")
...
QBCore.Commands.Add('creategarage', 'BB-Garages: Create New Garage', {{name = 'name', help = 'Name'}}, true, function(source, args)
...
QBCore.Commands.Add('setinteract', 'BB-Garages: Set Closest Garage Interact Location', {{name = 'enable', help = 'Enable Ped? [0/1]'}}, true, function(source, args)
    TriggerClientEvent('bb-garages:client:GetPlayerCoords', source, 'bb-garages:server:dev:setinteract', args)
end, GaragesConfig['settings']['dev']['rank'])

QBCore.Commands.Add('setpayment', 'BB-Garages: Set the payment for the closest garage', {{name = 'price', help = 'Price [Perhour/Onetime]'}, {name = 'onetime', help = 'Onetime? [0/1]'} }, true, function(source, args)
    local price = args[1]
...
QBCore.Commands.Add('setslots', 'BB-Garages: Toggle BB-Garages Slots Editor', {}, false, function(source, args)
	local src = source
