    ...
    if not HasKey then 
        ...
        if vehicle ~= nil and vehicle ~= 0 then
            if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
                ...
                if exports["kwk-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20)) ~= 100 then             
                    ...
                    if c == o then
                        TriggerServerEvent('kwk-hud:Server:GainStress', math.random(1, 4))
                    end
                    return
                end
                ...
                if exports["kwk-taskbarskill"]:taskBar(1500,math.random(5,15)) ~= 100 then
                    StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
                    HasKey = false
                    SetVehicleEngineOn(vehicle, false, false, true)
                    QBCore.Functions.Notify("Lockpicking failed!", "error")
                ...
                IsHotwiring = false
                TriggerServerEvent('kwk-hud:Server:GainStress', math.random(2, 4))
...
function Hotwire()
    if not HasKey then 
        ...
        if not isParked then
            ...
            QBCore.Functions.Progressbar("hotwire_vehicle", "Attempting Hotwire", hotwireTime, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
