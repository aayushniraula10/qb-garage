...
BBGarages.Functions = {
    TriggerNUI = (function(onVehicle, name, data, key)
        if key == 'garages' then
            QBCore.Functions.TriggerCallback('bb-garages:server:getOwnedVehicles', function(vehicles)
                while not vehicles do Wait(0) end
    
                if onVehicle then
                    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
                    local vehicleProps = QBCore.Functions.GetVehicleProperties(vehicle)
                    QBCore.Functions.TriggerCallback('bb-garages:server:isVehicleOwned', function(owned)
                        if owned then
                            SendNUIMessage({
                                open = true,
                                type = "open-garage",
        ...
        elseif key == 'impounds' then
            QBCore.Functions.TriggerCallback('bb-garages:server:hasFines', function(hasfines)
                if hasfines == false then
                    QBCore.Functions.TriggerCallback('bb-garages:server:getImpoundedVehicles', function(vehicles)
                        while not vehicles do Wait(0) end
